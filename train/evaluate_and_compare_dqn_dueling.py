...
def setup_logging():
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_file = os.path.join(log_dir, f'evaluate_dqn_dueling_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    logging.basicConfig(
        level=logging.INFO,
...
def compare_models():
    ...
    dueling_agent1 = DuelingDQNAgent(state_dim, action_dim, device=device, logger=logger)
    dueling_agent2 = DuelingDQNAgent(state_dim, action_dim, device=device, logger=logger)

    # Load model weights
    model_paths = {
        'DQN1': '../splendor/models/dqn_model_final.pth',
        'DQN2': '../splendor/models/dqn_model_final.pth',
        'DuelingDQN1': '../splendor/models/dueling_model_final.pth',
        'DuelingDQN2': '../splendor/models/dueling_model_final.pth'
    }
    ...
    for name, path in model_paths.items():
        if os.path.exists(path):
            if name.startswith('DQN'):
                ...
                checkpoint = torch.load(path, map_location=device)
                if name == 'DuelingDQN1':
                    dueling_agent1.q_network.load_state_dict(checkpoint['q_network_state_dict'])
                    dueling_agent1.target_network.load_state_dict(checkpoint['target_network_state_dict'])
                    agents.append(dueling_agent1)
                else:
                    dueling_agent2.q_network.load_state_dict(checkpoint['q_network_state_dict'])
                    dueling_agent2.target_network.load_state_dict(checkpoint['target_network_state_dict'])
                    agents.append(dueling_agent2)
